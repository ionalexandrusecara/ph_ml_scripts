...
mydb = myclient["posture_training_right"]
...
nothingDB = myclient["posture_training_2"]
nothing_col = nothingDB["nothing"]

#Retrieve all documents from MongoDB collection
all_forward_head = pd.DataFrame(list(forward_head_col.find()))
all_healthy_posture = pd.DataFrame(list(healthy_posture_col.find()))
all_left_head_flexion = pd.DataFrame(list(left_head_flexion_col.find()))
all_left_head_rotation = pd.DataFrame(list(left_head_rotation_col.find()))
all_left_shoulder_elevation = pd.DataFrame(list(left_shoulder_elevation_col.find()))
#all_left_torso_rotation = pd.DataFrame(list(left_torso_rotation_col.find()))
all_nothing = pd.DataFrame(list(nothing_col.find())).head(50)
all_right_head_flexion = pd.DataFrame(list(right_head_flexion_col.find()))
all_right_head_rotation = pd.DataFrame(list(right_head_rotation_col.find()))
all_right_shoulder_elevation = pd.DataFrame(list(right_shoulder_elevation_col.find()))
#all_right_torso_rotation = pd.DataFrame(list(right_torso_rotation_col.find()))
all_tucked_chin = pd.DataFrame(list(tucked_chin_col.find()))
...
with open('right_ph_trained_model.pkl', 'wb') as f:
    pickle.dump(classifier, f)
    
with open('right_ph_trained_model.pkl', 'rb') as f:
